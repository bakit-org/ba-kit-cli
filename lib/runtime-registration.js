...
function isManagedCodexHookCommand(hook, root, commands) {
  if (!hook || typeof hook !== 'object') return false;
  const command = String(hook.command || '');
  const normalizedCommand = command.replace(/\\/g, '/');
  const normalizedRoot = path.join(root, 'ba-kit', 'hooks').replace(/\\/g, '/');
  const legacyManaged = (
    normalizedCommand.includes(`${normalizedRoot}/`) ||
    normalizedCommand.includes('/ba-kit/hooks/')
  ) && /\/(?:guardrail-[^/"\s]+|postwrite-guardrail)\.sh(?:["\s]|$)/.test(normalizedCommand);
  return (
...
function filterCodexHookEntries(entries, root, commands) {
  return entries.flatMap(entry => {
    if (!entry || typeof entry !== 'object') return [entry];
    if (isManagedCodexHookCommand(entry, root, commands)) return [];
    if (!Array.isArray(entry.hooks)) return [entry];
    const hooks = entry.hooks.filter(hook => !isManagedCodexHookCommand(hook, root, commands));
    return hooks.length ? [{ ...entry, hooks }] : [];
  });
}

function codexHookEntriesContain(entries, command) {
  return (entries || []).some(entry => {
    if (!entry || typeof entry !== 'object') return false;
    if (String(entry.command || '') === command) return true;
    return Array.isArray(entry.hooks) && entry.hooks.some(hook => String(hook && hook.command || '') === command);
  });
}
...
function registerCodex(root, agentRegistration, hookRegistration, allowLegacy = false) {
  ...
  for (const [event, entries] of Object.entries(descriptor.hooks || {})) {
    ...
    hooks.hooks[event] = filterCodexHookEntries(hooks.hooks[event], root, new Set(commands));
...
function unregisterCodex(root) {
  ...
  if (hooks && hooks.hooks) {
    ...
    for (const [event, entries] of Object.entries(hooks.hooks)) {
      if (Array.isArray(entries)) hooks.hooks[event] = filterCodexHookEntries(entries, root, commands);
    }
    writeJson(hooksFile, hooks);
  }
  return [configFile, hooksFile];
...
function registrationHealthy(runtime, root) {
  ...
  if (runtime === 'codex') {
    ...
    const hasHooks = Object.entries(descriptor.hooks || {}).every(([event, entries]) => entries.every(expected => {
      ...
      return codexHookEntriesContain(hooks.hooks[event], command);
    }));
