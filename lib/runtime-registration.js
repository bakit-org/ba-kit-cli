...
function unregisterCodex(root) {
  ...
  if (fs.existsSync(configFile)) {
    ...
    const end = '# <<< BA-kit managed agents <<<';
    const escaped = new RegExp(`\\n?${begin.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[\\s\\S]*?${end.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\n?`, 'g');
    let config = fs.readFileSync(configFile, 'utf8').replace(escaped, '\n');
...
function registrationHealthy(runtime, root) {
  ...
  if (runtime === 'codex') {
    ...
    const hasAgents = config.includes('# >>> BA-kit managed agents >>>') && (agents.canonical_agents || []).every(name => config.includes(`[agents.${name}]`));
