...
function isReplaceableComponent(runtime, key, target, component, dest) {
  return runtime === 'claude'
    && key === './.local/bin/ba-kit'
    && component === 'claude-cli'
    && path.resolve(dest) === path.resolve(target, 'ba-kit', 'ba-kit');
}
...
function targetMetadataFiles(target) {
  return [
    path.join(target, 'ba-kit', 'state.json'),
    path.join(target, 'ba-kit', 'VERSION'),
    path.join(target, 'ba-kit', 'PRODUCT'),
    path.join(target, 'ba-kit', 'PRODUCT_ID'),
    path.join(target, 'ba-kit', 'release-manifest.json'),
    path.join(target, 'ba-kit', BARRIER),
    preferenceFile(target),
  ];
}

function registrationFiles(runtime, target) {
  if (runtime === 'claude') return [path.join(target, 'settings.json')];
  if (runtime === 'codex') return [path.join(target, 'config.toml'), path.join(target, 'hooks.json')];
  return [];
}

function isManagedClaudeCoreLink(target) {
  const link = path.join(target, 'core');
  if (!fs.lstatSync(link, { throwIfNoEntry: false })?.isSymbolicLink()) return false;
  const resolvedTarget = path.resolve(path.dirname(link), fs.readlinkSync(link));
  return resolvedTarget === path.resolve(target, 'ba-kit', 'core');
}
...
function restoreRecord(record, home, transactionRoot, allowedRoots = null) {
  ...
  if (record.kind === 'directory') { ensureDir(record.file); return; }
  rm(record.file);
...
function pruneEmptyAncestors(start, boundary) {
  let cursor = path.resolve(start);
  const stop = path.resolve(boundary);
  while (cursor !== stop && within(stop, cursor)) {
    try { fs.rmdirSync(cursor); } catch (error) {
      if (error.code === 'ENOENT') { cursor = path.dirname(cursor); continue; }
      if (error.code === 'ENOTEMPTY' || error.code === 'EEXIST' || error.code === 'ENOTDIR') break;
      throw error;
    }
...
function buildState(ctx, ops, statuses, retired) {
  ...
  for (const op of ops) {
    ...
    files[op.key].source_sha256 = op.desired;
    files[op.key].component_id = op.component;
    files[op.key].destinations = files[op.key].destinations || {};
  ...
  try {
    for (const op of ops) {
      if (exists(op.dest)) {
        ...
        else if (isReplaceableComponent(ctx.runtime, op.key, ctx.target, op.component, op.dest)) { atomicCopy(op.source, op.dest); statuses.set(op.dest, 'managed'); }
...
function uninstallOne(ctx, state) {
  if (!state || !state.files) return [];
  const preserved = [];
  const removableParents = new Set([path.join(ctx.target, 'ba-kit')]);
  for (const [key, entry] of Object.entries(state.files)) for (const [dest, data] of Object.entries(entry.destinations || {})) {
    if (!within(ctx.target, dest)) continue;
    assertManagedDestination(ctx.home, ctx.runtime, dest);
    removableParents.add(path.dirname(dest));
    if (!exists(dest)) continue;
    const current = sha256(dest); const expected = data.installed_sha256 || data.source_sha256;
    if (isReplaceableComponent(ctx.runtime, key, ctx.target, entry.component_id, dest)
      || (data.status === 'managed' && current === expected)) rm(dest);
    else preserved.push({ key, dest, hash: current, component: entry.component_id, status: 'preserved-modified' });
  }
  if (ctx.runtime === 'claude') unregisterClaude(ctx.target);
  if (ctx.runtime === 'codex') unregisterCodex(ctx.target);
  const targetMetadata = targetMetadataFiles(ctx.target);
  for (const file of targetMetadata) { assertManagedDestination(ctx.home, ctx.runtime, file); rm(file); }
  if (ctx.runtime === 'claude' && isManagedClaudeCoreLink(ctx.target)) rm(path.join(ctx.target, 'core'));
  for (const dir of [...removableParents].sort((a, b) => b.length - a.length)) pruneEmptyAncestors(dir, ctx.target);
  return preserved;
}

function finalizeUninstallState(home, runtime, state, preserved) {
  const stateFile = path.join(home, '.local', 'share', 'ba-kit', 'runtime-state', runtime, 'state.json');
  if (preserved.length) {
...
function uninstall(args) {
  ...
  const runtimes = [...new Set(String(args.runtimes || '').split(',').filter(Boolean))];
  for (const runtime of runtimes) for (const target of runtimeTargets(runtime, home)) discoveredContexts.push({ runtime, target, home });
  const runtimeStates = new Map();
  for (const runtime of runtimes) {
    let state = readJson(path.join(home, '.local', 'share', 'ba-kit', 'runtime-state', runtime, 'state.json'), null);
    if (!state) for (const ctx of discoveredContexts.filter(item => item.runtime === runtime)) {
      state = readJson(path.join(ctx.target, 'ba-kit', 'state.json'), null);
      if (state) break;
    }
    runtimeStates.set(runtime, state);
  }
  const contexts = discoveredContexts.filter(ctx => {
    const targets = runtimeStates.get(ctx.runtime)?.targets;
    return !Array.isArray(targets) || targets.some(target => path.resolve(target) === ctx.target);
  });
  ...
  const paths = [];
  for (const ctx of contexts) {
    const state = runtimeStates.get(ctx.runtime);
    for (const entry of Object.values((state && state.files) || {})) for (const dest of Object.keys(entry.destinations || {})) {
      assertManagedDestination(home, ctx.runtime, dest);
      if (within(ctx.target, dest)) paths.push(dest);
    }
    paths.push(path.join(home, '.local', 'share', 'ba-kit', 'runtime-state', ctx.runtime, 'state.json'));
    const targetPaths = [
      ...targetMetadataFiles(ctx.target),
      ...registrationFiles(ctx.runtime, ctx.target),
    ];
    for (const file of targetPaths) assertManagedDestination(home, ctx.runtime, file);
    paths.push(...targetPaths);
    if (ctx.runtime === 'claude') {
      const core = path.join(ctx.target, 'core');
      if (isManagedClaudeCoreLink(ctx.target)) {
        assertManagedDestination(home, ctx.runtime, core);
        paths.push(core);
      }
    }
    const directoryPaths = new Set([path.join(ctx.target, 'ba-kit')]);
    for (const file of paths.filter(candidate => within(ctx.target, candidate))) {
      let dir = path.dirname(file);
      while (dir !== ctx.target && within(ctx.target, dir)) { directoryPaths.add(dir); dir = path.dirname(dir); }
    }
  ...
  try {
    for (const runtime of runtimes) {
      ...
      if (!state || !state.files) continue;
      const preserved = [];
      for (const ctx of contexts.filter(item => item.runtime === runtime)) preserved.push(...uninstallOne(ctx, state));
      finalizeUninstallState(home, runtime, state, preserved);
    }
    journal.status = 'committed'; writeJson(path.join(transactionRoot, 'journal.json'), journal); rm(transactionRoot);
  } catch (error) {
