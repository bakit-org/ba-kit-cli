...
function loadContext(args) {
  ...
  return { extract, manifest, release, home: path.resolve(args.home || process.env.HOME) };
}
...
function writeBarrier(target, runtime) {
  ...
  writeJson(file, { status: 'cli-upgrade-required', runtime, min_cli_version: '1.4.0', managed_by: 'ba-kit-cli-v3' });
}
...
function installOne(ctx) {
  ...
  const paths = [...ops.map(op => op.dest), ...registrationFiles, stateFile, projection, barrier, manifestFile, ...metadataFiles];
...
function install(args) {
  ...
  for (const ctx of contexts) {
    ...
    paths.push(
      ...
      path.join(ctx.target, 'ba-kit', 'PRODUCT'), path.join(ctx.target, 'ba-kit', 'PRODUCT_ID'),
    );
    if (ctx.runtime === 'claude') {
...
function uninstallOne(ctx) {
  ...
  if (preserved.length) {
    state.status = 'uninstalled-with-preserved-files'; state.files = {};
...
function doctor(args) {
  ...
  for (const runtime of String(args.runtimes || 'claude,codex,agy').split(',').filter(Boolean)) for (const target of runtimeTargets(runtime, home)) {
    ...
    installed += 1;
    if (!validV3State(state, runtime)) { console.log(`[MIGRATE] ${runtime}: schema-v3 state invalid at ${target}`); failed = true; continue; }
    ...
    console.log(`${bad ? '[FAIL]' : '[OK]'} ${runtime}: schema-v3 payload=${state.payload_schema} target=${target} issues=${bad}${bad ? ' repair=ba-kit update' : ''}`);
    if (bad) failed = true;
  }
