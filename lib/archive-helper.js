...
function parseArgs() {
  const args = { profile: null, archive: null, destination: null, cliVersion: null, selectedProduct: null, selectedVersion: null };
...
async function main() {
  ...
  if (cmd === 'inspect') {
    if (!args.archive || !args.profile || !args.cliVersion) {
      console.error('Usage: archive-helper.js inspect --archive A --profile P --cli-version V [--selected-product ID] [--selected-version TAG]');
      process.exit(1);
    }
